...
const Head = () => {
  ...
  return (
    <div className=" relative px-4 py-2 flex justify-between items-center shadow-sm  w-full sticky top-0 z-10 bg-white h-[4.62rem] dark:bg-zinc-900 dark:text-white transition-all duration-500 ">
      ...
      <div className="center w-3/5 2xl:w-2/5  max-sm:ml-2 max-sm:mr-4 flex items-center ml-16 relative ">
        ...
        >
          <input
            ...
            placeholder="Search"
            value={searchQuery}
            className=" rounded-l-3xl p-2 md:pl-8 focus:outline-none w-full dark:bg-zinc-800"
      ...
      {/* SUGGESTION DIV */}
      {showSuggestions && (
        <div className="absolute -mx-4 md:mx-0 sm:left-[23%] top-[80%] bg-white py-2 px-2 w-[36rem] shadow-lg rounded-2xl border border-gray-200 dark:bg-zinc-900 ">
          <ul>
            {suggestions.map((s) => (
              <li
                key={s}
      ...
      {/* --------------------------------------------- */}
      <div className="right-menu flex  items-center sm:ml-4 lg:ml-16 gap-[3%] p-2">
        ...
        <div className="p-2 max-md:hidden  hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-full cursor-pointer">
