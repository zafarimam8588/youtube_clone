...
import loadingGif from "../assets/loading-state.gif";
...
const Comments = ({videoId,commentCount}) => {
  const getComments = async (nextPageToken = "") => {
    const response = await fetch(
      BASE_URL +
        `/commentThreads?part=snippet%2Creplies&maxResults=18&order=relevance&key=${ process.env.REACT_APP_GOOGLE_API_KEY_5}&videoId=${videoId}&textFormat=plainText&pageToken=${
          nextPageToken ?? ""
        }`
    );
  ...
  return isLoading ? (
    ...
    <div className="">
      ...
      <div className=" text-sm flex items-center  gap-4 mt-8 mb-4">
        <div className="">
